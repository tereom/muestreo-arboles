...
```{r, fig.height=4, fig.width=5.5, fig.align='center', warning=FALSE}
...
```{r, fig.height=8, fig.width=5.5, fig.align='center', warning=FALSE}
medias_1 <- muestreo_1 %>%
...
rep_aleatorio <- rdply(2000, aleatorio_1)
...
ggplot(aleatorio_g, aes(x = reorder(1:nrow(aleatorio_g), medicion_media), 
  ...
  scale_color_brewer(palette = "Reds")  +
  scale_x_discrete("Conglomerados", labels = "") +
  labs(title = "Medición muestras aleatorias", color = "# árboles") +
  facet_wrap(~ variable, ncol = 1, scales = "free")
...
Notamos que es más difícil estimar la variable diámetro; esto nos habla de 
una dependencia espacial en las medicones a nivel sitio, sugiriendo que 
ponderemos por la variable número de árboles.
