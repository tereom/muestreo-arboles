...
Concentrémonos en 6 conglomerados elegidos al azar:
...
```{r, fig.height=5, fig.width=7.5, fig.align='center', cache=TRUE}
...
se_aleatorio <- rep_aleatorio %>% 
  ...
  summarise(altura_media = mean(altura_2), # promedio de las alturas
    altura_05 = quantile(altura_2, 0.05, na.rm = TRUE),
    altura_95 = quantile(altura_2, 0.95, na.rm = TRUE)
  )

aleatorio_g <- left_join(se_aleatorio, medias_1)

ggplot(aleatorio_g, aes(x = reorder(1:nrow(aleatorio_g), altura_media), 
  y = altura_media, ymin = altura_05, 
  ymax = altura_95)) +
...
Ahora comparamos con las verdaderas medias, recordemos que son intervalos de
90% por lo que esperamos que el 10% se sitúen fuera del intervalo.

```{r, fig.height=5, fig.width=7.5, fig.align='center', cache=TRUE}
aleatorio_congs_sum <- select(arbolado_resumen, id = IdConglomerado, 
  altura_pob = altura_media)
aleatorio_g2 <- left_join(aleatorio_g, aleatorio_congs_sum)
ggplot(aleatorio_g2, aes(x = reorder(1:nrow(aleatorio_g), altura_media), 
  y = altura_media, ymin = altura_05, ymax = altura_95)) +
  geom_pointrange(aes(color = num_cat)) +
  geom_point(aes(y = altura_pob), alpha = 0.8, size = 1.5) +
  scale_color_brewer(palette = "Reds")  +
  scale_x_discrete("Conglomerados", labels = "") +
  labs(title = "Altura muestras aleatorias", color = "# árboles")
```
